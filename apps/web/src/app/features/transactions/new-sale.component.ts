...
@Component({
  ...
  imports: [
    ...
    MatChipsModule, MatListModule, PageHeaderComponent,
  ],
  template: `
      ...
      <mat-card-content>
        <div style="margin-bottom: 24px;">
          ...
          <mat-form-field appearance="outline" class="full-width">
            ...
            <input matInput [matAutocomplete]="auto" [(ngModel)]="customerSearch" (ngModelChange)="onCustomerSearch($event)">
          </mat-form-field>

          <mat-autocomplete #auto="matAutocomplete" (optionSelected)="onCustomerSelect($event.option.value)">
          ...
          @if (selectedCustomer(); as customer) {
            <mat-chip [color]="'primary'" style="margin-top: 8px;">
              {{ customer.name }} — Debt: {{ customer.totalDebt | number:'1.2-2' }}
            </mat-chip>
          }
        </div>
        ...
        @if (selectedCustomer(); as customer) {
          ...
          <div style="margin-bottom: 24px;">
            ...
            <mat-form-field appearance="outline" class="full-width">
              ...
              <input matInput [(ngModel)]="productSearch" (ngModelChange)="onProductSearch($event)">
            </mat-form-field>

            <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 16px;">
              @for (p of filteredProducts(); track p.id) {
                <mat-chip (click)="addProduct(p)" style="cursor: pointer;">
                  {{ p.name }} — {{ p.price | number:'1.2-2' }} ({{ p.stockQuantity }})
                </mat-chip>
              }
            </div>
            ...
            @for (item of selectedItems(); track item.product.id) {
              <div style="display: flex; align-items: center; gap: 12px; padding: 8px 0;">
                ...
                <input type="number" [ngModel]="item.quantity" (ngModelChange)="updateQuantity(item.product.id, $event)"
                  style="width: 60px; padding: 4px; border: 1px solid #ccc; border-radius: 4px; text-align: center;" min="1">
            ...
            <p style="text-align: right; font-size: 18px; font-weight: 500; margin-top: 16px;">
          ...
          <div>
            ...
            <button mat-raised-button color="primary" style="width: 100%; padding: 8px;"
              [disabled]="!selectedItems().length || submitting()" (click)="onSubmit()">
              {{ submitting() ? 'Creating sale...' : 'Confirm Sale — ' + (totalAmount() | number:'1.2-2') }}
            </button>
          </div>
        }
      </mat-card-content>
...
export class NewSaleComponent implements OnInit {
  ...
  protected totalAmount = computed(() =>
    this.selectedItems().reduce((sum, item) => sum + Number(item.product.price) * item.quantity, 0),
  );

  constructor() {
    this.customerSearch$.pipe(
      debounceTime(300), distinctUntilChanged(), takeUntilDestroyed(inject(DestroyRef)),
    ).subscribe((q) => this.api.customers.getAll({ search: q || undefined }).then((r) => this.customers.set(r.data)));

    this.productSearch$.pipe(
      debounceTime(300), distinctUntilChanged(), takeUntilDestroyed(inject(DestroyRef)),
    ).subscribe((q) => this.api.products.getAll({ search: q || undefined }).then((r) => this.products.set(r.data)));
  }
  ...
  addProduct(product: IProduct) {
    ...
    const exists = this.selectedItems().find((i) => i.product.id === product.id);
    if (exists) {
  ...
  updateQuantity(productId: string, qty: number) {
    ...
    this.selectedItems.update((items) =>
      items.map((i) => (i.product.id === productId ? { ...i, quantity: num } : i)),
    );
  }
  ...
  async onSubmit() {
    ...
    try {
      await this.api.transactions.createSale({
        ...
        items: items.map((i): ISaleItem => ({ productId: i.product.id, quantity: i.quantity })),
